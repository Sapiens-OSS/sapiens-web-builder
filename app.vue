    ...
    <div class="hidden w-28 overflow-y-auto bg-slate-700 dark:bg-slate-900 dark:border-r dark:border-gray-700 md:block">
      <div class="flex w-full flex-col items-center py-6">
        <div class="flex flex-shrink-0 items-center">
          <img class="h-8 w-auto" src="@/assets/img/logo.png" alt="Sapiens Web Builder" />
        </div>
        <div class="mt-6 w-full flex-1 space-y-1 px-2">
          <NuxtLink v-for="(item, i) in pages" :key="item.name" :to="item.href" :class="[
            selectedTab == i
      ...
      <Dialog as="div" class="relative z-20 md:hidden" @close="mobileMenuOpen = false">
        ...
        <div class="fixed inset-0 z-40 flex">
          <TransitionChild as="template" enter="transition ease-in-out duration-300 transform"
            ...
            <DialogPanel class="relative flex w-full max-w-xs flex-1 flex-col bg-slate-700 dark:bg-slate-800 pt-5 pb-4">
              ...
              <div class="mt-5 h-0 flex-1 overflow-y-auto px-2">
                <nav class="flex h-full flex-col">
                  <div class="space-y-1">
                    ...
                    ]" :aria-current="selectedTab == i ? 'page' : undefined" @click="mobileMenuOpen = false">
                      <component :is="item.icon" :class="[
                        selectedTab == i
                          ? 'text-white'
                          : 'text-slate-300 group-hover:text-white',
